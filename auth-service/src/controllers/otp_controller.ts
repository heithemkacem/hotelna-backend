...
const validateOTP = async (req: Request, res: Response) => {
    try {
      ...
      if (!otpRecord) {
        return res.json({ ok: false, status: 'Failed', message: 'Invalid OTP or OTP has expired.' });
      }
      ...
      if (!profile) {
        return res.json({ ok: false, status: 'Failed', message: 'User not found.' });
      }
  ...
  const forgetPassword = async (req: Request, res: Response) => {
    try {
      ...
      const profile = await Profile.findOne({ email });
      if (!profile) {
        return res.json({ ok: false, status: 'Failed', message: 'Email does not exist.' });
      }
      ...
      await OTP.create({ email, otp: hashedOTP, type: 'reset-password', createdAt: new Date() });
